...
export function ParquesAtraccionesContent({
  ...
  return (
    <div className={isModal ? "min-h-0 flex-1 overflow-y-auto px-3 py-6 sm:px-4 scrollbar-modal" : "container mx-auto px-4 py-12"}>
      ...
      {intro ? (
        <p
          className={`text-sm text-gray-700 leading-relaxed mb-6 border-l-2 border-brand-gold whitespace-pre-line ${isModal ? "pl-3" : "pl-4"}`}
        >
          {intro}
        </p>
      ) : null}
      ...
      <div className="grid grid-cols-1 gap-4">
        {rowsWithIds.map(({ group, mainId, bulletIds }, idx) => (
          <div
            ...
            id={mainId}
            className={`bg-brand-bg border border-gray-100 rounded-sm flex flex-col gap-3 hover:border-brand-gold/40 transition-colors scroll-mt-6 ${isModal ? "p-4 sm:p-5" : "p-5"}`}
          >
            <>
