...
const LINK_ENTRIES = Object.entries(LINK_TEXT_TO_URL).sort((a, b) => b[0].length - a[0].length);
...
function parseVoiceBullets(
  ...
  for (let i = 0; i < bullets.length; i++) {
    const b = bullets[i];
    const isQuote = b.startsWith('"') || b.startsWith("\u201C") || b.startsWith("\u00AB") || b.startsWith("\u201E");
    if (isQuote) {
      const next = bullets[i + 1] ?? "";
      const isAttrib = next.startsWith("—") || next.startsWith("–") || next.startsWith("-");
        ...
        <div className="overflow-y-auto flex-1 px-8 py-6 scrollbar-modal">
          ...
          <div className="grid grid-cols-1 gap-4">
            {groups.map((group, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-4">
                      {pairs.map((pair, pIdx) => (
                        <div key={pIdx} className="flex flex-col gap-1.5 pl-4 border-l-2 border-brand-gold/50">
              ...
              const [mainHeader, mainBody] = newlineIdx !== -1
                  ...
                  {mainHeader ? (
                    ...
                    <p className="text-sm text-gray-800">
                      {(() => {
                        ...
                        if (ci === -1) return <strong>{renderLinkedText(group.main)}</strong>;
                        return (
                          <>
