...
const placeNamesRegex = new RegExp(
  `(${placeNames
...
function renderTextWithBoldPlaces(text: string) {
  const parts = text.split(placeNamesRegex);
  return parts.map((part, idx) =>
    placeNames.includes(part) ? <strong key={`${part}-${idx}`}>{part}</strong> : part,
  );
}

const sectionsMeta = [
  { key: "expBBQModalSection1" },
  { key: "expBBQModalSection2" },
  { key: "expBBQModalSection3" },
  { key: "expBBQModalSection4" },
] as const;
...
export default function BBQParksModal({ isOpen, onClose }: BBQParksModalProps) {
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="bbq-modal-title"
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="relative bg-brand-bg border-b border-gray-200 px-8 py-6">
          ...
          <h3 id="bbq-modal-title" className="text-3xl font-serif text-gray-900 leading-tight">
            {t("expBBQModalTitle")}
          </h3>
          <button
            type="button"
            ...
            {sectionsMeta.map(({ key }, index) => (
              ...
              >
                ...
                <p className="text-xs text-gray-700 leading-relaxed">
                  {renderTextWithBoldPlaces(t(key))}
                </p>
              </div>
            ))}
          </div>
