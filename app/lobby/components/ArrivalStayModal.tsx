...
export function ArrivalStayContent({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`flex shrink-0 items-start justify-between gap-4 border-b border-gray-100 bg-linear-to-br from-brand-bg via-white to-amber-50/30 py-6 ${isModal ? "px-3 sm:px-4" : "px-8"}`}
        >
          <div>
        ...
        <div className="scrollbar-modal min-h-0 flex-1 overflow-y-auto">
          {/* Intro */}
          <div
            className={`space-y-3 py-6 text-[15px] font-light leading-relaxed text-gray-600 border-b border-gray-100 ${isModal ? "px-3 sm:px-4" : "px-8"}`}
          >
            <p className={BODY}>{t("lobStayIntro1")}</p>
            <p className={BODY}>{t("lobStayIntro2")}</p>
          </div>

          <div className={`space-y-10 ${isModal ? "p-3 sm:p-4" : "p-8"}`}>
            {/* 3 steps */}
            <div>
