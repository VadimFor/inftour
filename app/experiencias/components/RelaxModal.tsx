...
function renderLabeledLine(line: string) {
  ...
  return (
    <>
      <span className="font-semibold">{keyword}</span>{detail}: {rest}
    </>
  );
}
...
function renderMainLine(line: string) {
  ...
  return <><strong>{boldPart}</strong>{rest ? " " + rest : ""}</>;
}
...
export default function RelaxModal({ isOpen, onClose }: RelaxModalProps) {
  ...
  for (const line of contentLines) {
    ...
    const isTimeSeasonBullet =
      isTimeOrSeasonBullet(line) && !locationUnit;
            ...
            {sections.map((section, sIdx) => (
              ...
              >
                ...
                {section.subgroups.map((sg, sgIdx) => (
                  <div key={`sg-${sgIdx}-${sg.main.slice(0, 20)}`} className="flex flex-col gap-2">
                    <p className="text-sm text-gray-900">{renderMainLine(sg.main)}</p>
                    {sg.bullets.length > 0 && (
