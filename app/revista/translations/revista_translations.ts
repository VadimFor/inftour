...
export const revistaTranslations: Record<string, LangStrings> = {
  ...
  revistaPlaceholderIntro: {
    eng: "Magazine PDFs use public Google Drive links: add each file’s id and title in app/revista/config/magazineDrivePdfs.ts (see app/revista/config/README.txt). No API key is required.",
    esp: "Los PDF usan enlaces públicos de Google Drive: añade el id y el título de cada archivo en app/revista/config/magazineDrivePdfs.ts (ver app/revista/config/README.txt). No hace falta clave de API.",
    ru: "PDF журналов — публичные ссылки Google Drive: добавьте id и название каждого файла в app/revista/config/magazineDrivePdfs.ts (см. app/revista/config/README.txt). Ключ API не нужен.",
    fr: "Les PDF utilisent des liens Google Drive publics : ajoutez l’id et le titre de chaque fichier dans app/revista/config/magazineDrivePdfs.ts (voir app/revista/config/README.txt). Aucune clé API.",
    it: "I PDF usano link pubblici di Google Drive: aggiungi id e titolo di ogni file in app/revista/config/magazineDrivePdfs.ts (vedi app/revista/config/README.txt). Nessuna chiave API.",
    de: "Magazin-PDFs nutzen öffentliche Google-Drive-Links: tragen Sie je Datei ID und Titel in app/revista/config/magazineDrivePdfs.ts ein (siehe app/revista/config/README.txt). Kein API-Schlüssel nötig.",
    uk: "PDF журналів — публічні посилання Google Drive: додайте id і назву кожного файлу в app/revista/config/magazineDrivePdfs.ts (див. app/revista/config/README.txt). Ключ API не потрібен.",
    pl: "PDF-y korzystają z publicznych linków Google Drive: dodaj id i tytuł każdego pliku w app/revista/config/magazineDrivePdfs.ts (zob. app/revista/config/README.txt). Bez klucza API.",
  },
  revistaSpread1Title: {
  ...
  revDriveArchiveTitle: {
    eng: "Magazine PDF archive",
    esp: "Archivo de revistas (PDF)",
    ru: "Архив PDF журналов",
    fr: "Archives des magazines (PDF)",
    it: "Archivio riviste (PDF)",
    de: "Magazin-PDF-Archiv",
    uk: "Архів PDF журналів",
    pl: "Archiwum magazynów (PDF)",
  },
  revDriveArchiveEmpty: {
    eng: "No PDFs listed yet. Add public Drive file ids in app/revista/config/magazineDrivePdfs.ts.",
    esp: "Aún no hay PDF en la lista. Añade los id públicos de Drive en app/revista/config/magazineDrivePdfs.ts.",
    ru: "В списке пока нет PDF. Добавьте публичные id файлов в app/revista/config/magazineDrivePdfs.ts.",
    fr: "Aucun PDF dans la liste. Ajoutez les id publics Drive dans app/revista/config/magazineDrivePdfs.ts.",
    it: "Nessun PDF in elenco. Aggiungi gli id pubblici di Drive in app/revista/config/magazineDrivePdfs.ts.",
    de: "Noch keine PDFs in der Liste. Tragen Sie öffentliche Drive-Datei-IDs in app/revista/config/magazineDrivePdfs.ts ein.",
    uk: "У списку ще немає PDF. Додайте публічні id файлів у app/revista/config/magazineDrivePdfs.ts.",
    pl: "Brak PDF na liście. Dodaj publiczne id plików w app/revista/config/magazineDrivePdfs.ts.",
  },
