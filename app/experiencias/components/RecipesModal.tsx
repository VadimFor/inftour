...
export default function RecipesModal({ isOpen, onClose }: RecipesModalProps) {
  ...
  const parseSection = (key: string) => {
    const normalized = t(key)
      .replace(/\)(?=[A-ZА-ЯІЇЄҐ])/g, ")\n")
      .replace(/[.。]?\s*[oо]\s+(?=(Step|Étape|Paso|Passo|Schritt|Krok|Шаг|Крок)\s+\d)/g, "\n○ ")
      .replace(
        /\n(Fuente:|Source:|Источник:|Джерело:|Fonte:|Quelle:|Źródło:)/g,
        "\n• $1",
      )
      .replace(/\n(El ritual)/g, "\n• $1");

    const stepKeywords = /^(Step|Étape|Paso|Passo|Schritt|Krok|Шаг|Крок)\s+\d/;
    const lines = normalized
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean)
      .map((line) => (stepKeywords.test(line) ? `○ ${line}` : line));

    const title = lines[0] ?? "";
    const description = lines[1] ?? "";
    const sourcePattern = /^[•]?\s*(Fuente:|Source:|Источник:|Джерело:|Fonte:|Quelle:|Źródło:)/;
    const allMeta = lines
      .slice(2)
      .filter(
        (line) =>
    ...
    const source = allMeta.filter((line) => sourcePattern.test(line));
    const ingredients = allMeta.filter((line) => !sourcePattern.test(line));
    const steps = lines.slice(2).filter((line) => line.startsWith("○"));
    const notes = lines
      .slice(2)
      .filter((line) => !line.startsWith("•") && !line.startsWith("○") && !sourcePattern.test(line));

    return { title, description, source, ingredients, steps, notes };
  };
                  ...
                  {section.source.length > 0 && (
                    <div className="bg-white/80 border border-gray-200 rounded-sm px-4 py-3">
                      <ul className="space-y-1 text-sm text-gray-500">
                        {section.source.map((line) => (
                          <li key={line} className="flex items-start gap-2">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-4 h-4 shrink-0 mt-0.5 text-gray-400" aria-hidden>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                            </svg>
                            <span className="leading-relaxed">{renderLine(line)}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {section.ingredients.length > 0 && (
                    <div className="bg-white/80 border border-brand-gold/20 rounded-sm p-4">
                      <div className="flex items-center gap-2 mb-3">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-4 h-4 text-brand-gold" aria-hidden>
                        ...
                        <span className="text-xs font-bold uppercase tracking-[0.15em] text-brand-gold">
                          {t("expRecipesModalIngredientsLabel") || "Ingredients"}
                        </span>
                      </div>
                      <ul className="space-y-1.5 text-sm text-gray-700">
                        {section.ingredients.map((line) => (
                          <li key={line} className="flex items-start gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-brand-gold shrink-0 mt-1.5" aria-hidden />
                            <span className="leading-relaxed">{renderLine(line)}</span>
                          </li>
                        ))}
                      </ul>
                  ...
                  {section.steps.length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold uppercase tracking-[0.15em] text-brand-gold">
                          {t("expRecipesModalStepsLabel") || "Steps"}
                        </span>
                      </div>
                      <ol className="relative ml-[18px] space-y-0">
                        {section.steps.map((step, i) => {
                          const { body } = parseStepNumber(step);
